...
Always provide accurate, educational, and scientifically correct information. If you cannot identify the exact species, provide your best assessment with the closest match.`;
    ...
    const isValidImage = image && typeof image === "string" && image.startsWith("data:image/") && image.length > 100;
    ...
    if (isValidImage) {
      ...
      return new Response(JSON.stringify({ error: "Please provide a valid image or species name to identify." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    console.log("Calling Lovable AI for species identification...", isValidImage ? "with image" : `for "${speciesName}"`);
    ...
    try {
      ...
      speciesInfo = {
        ...
        taxonomy: { kingdom: "Unknown", phylum: "Unknown", class: "Unknown", order: "Unknown", family: "Unknown", genus: "Unknown", species: "Unknown" },
